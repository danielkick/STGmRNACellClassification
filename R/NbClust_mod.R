NbClust_mod <-function(data = NULL, diss=NULL, distance ="euclidean", min.nc=2, max.nc=15, method =NULL, index = "all", alphaBeale = 0.1, max.tol = 1e-20)
{
...
Indices.WBT <- function(x,cl,P,s,vv)
{
  ...
  xbar <- solve(t(z)%*%z, tol = max.tol)%*%t(z)%*%x
  B <- t(xbar)%*%t(z)%*%z%*%xbar
  W <- P-B
  marriot <- (qq^2)*det(W)
  ...
  friedman <- sum(diag(solve(W, tol = max.tol)*B))
