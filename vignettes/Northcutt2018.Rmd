...
```{r}
function (obj, nClust, clMethods = c("clara", "diana", "hierarchical", 
    ...
    ...) 
{
    ...
    if ("all" %in% clMethods) {
        if (isTRUE(countData)) {
            clMethods <- c("em.nbinom", "da.nbinom", "sa.nbinom", 
                "em.poisson", "da.poisson", "sa.poisson")
        }
        else {
            if (hierMethod == "ward") {
                clMethods <- c("agnes", "clara", "diana", "fanny", 
                  "hierarchical", "kmeans", "model", "pam", "som", 
                  "sota")
            }
            else {
                clMethods <- c("clara", "diana", "fanny", "hierarchical", 
                  "kmeans", "model", "pam", "som", "sota")
            }
        }
    }
    ...
    mbCountNames <- c("em.nbinom", "da.nbinom", "sa.nbinom", 
        "em.poisson", "da.poisson", "sa.poisson")
    ...
    if (length(mbCountMethods) > 0) {
        ...
        if (exists(c("Treatment"), where = addArgs)) {
            ...
            if (length(uniqueTreatment) == length(unique(uniqueTreatment))) {
                uniqueTreatment <- substr(uniqueTreatment, 1, 
                  nchar(uniqueTreatment) - 1)
            }
        }
    ...
    if (exists(c("maxitems"), where = clValid.args)) {
        ...
        clValid.args <- clValid.args[-which(names(clValid.args) == 
            "maxitems")]
    }
    ...
    if (length(clMethods) > 0) {
        clVObj <- do.call("clValid", c(list(obj, nClust, clMethods, 
            validation, method = hierMethod, annotation = annotation, 
            verbose = clVerbose, maxitems = maxitems), clValid.args))
        ...
        measNames <- c(if ("stability" %in% validation) c("APN", 
            "AD", "ADM", "FOM"), if ("internal" %in% validation) c("Connectivity", 
    ...
    if (isTRUE(mbCount)) {
        valMBCount <- mbCountVal(mat, Normalizer, uniqueTreatment, 
            validation, measNames, nClust, metric, neighbSize, 
            clVerbose, mbCountMethods, iter.max, TMP, annotation, 
            GOcategory, goTermFreq, dropEvidence)
    ...
    if (isTRUE(mbCount)) {
        ...
        meas <- array(dim = c(length(measNames), length(nClust), 
            length(clAlgs)))
    ...
    optimal.list <- do.call("RankAggreg", c(list(x = clusterOrder$ranks, 
        k = nList, weights = clusterOrder$weights, method = rankMethod, 
        distance = distance, importance = importance, verbose = rankVerbose), 
        RankAggreg.args))
    new("optCluster", inputData = mat, clVal = clVal, ranksWeights = clusterOrder, 
        rankAgg = optimal.list)
}
```
